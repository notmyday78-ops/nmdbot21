...
async function leaveGuild(
    ...
    try {
        const shard = manager.shards.first();
        ...
        await shard.eval(async (client, id) => {
            const guild = client.guilds.cache.get(id);

            if (!guild) {
                return {
                    success: false,
                    error: "Guild not found on this shard"
                };
            }
        ...
        try {
            ...
            if (
                ...
                const guildIds = [
                    ...
                    ...(Array.isArray(payload.guild_ids)
                        ? payload.guild_ids
                        : []),

                    ...(Array.isArray(payload.guildIds)
                        ? payload.guildIds
                        : [])
                ];
                ...
                for (const guildId of uniqueGuildIds) {
                    results.push(
                        await leaveGuild(manager, guildId)
                    );
                }
            ...
            if (event === "launch.cleanup") {
                ...
                for (const guild of manager.guilds.cache.values()) {
...
const server = createServer(
    async (req, res) => {
        if (req.method === "GET" && req.url === "/") {
            ...
            return res.end("NMDBot is online!");
        }
        ...
        if (
            ...
            return handleWebhook(manager, req, res);
        }
...
async function main() {
    ...
    console.log(
        "Discord recommends " + shards + " shard(s)."
    );
